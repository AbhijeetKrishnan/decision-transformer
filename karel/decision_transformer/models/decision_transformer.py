...
class DecisionTransformer(TrajectoryModel):
    ...
    def forward(
        ...
        if self.use_seq_state_embedding:
            ...
            )  # for compatibility with Linear layer
        ...
        masked_action_categoricals = [
            CategoricalMasked(logits=action, mask=action_mask).logits
            for (action, action_mask) in zip(action_preds, action_masks)
        ]
    ...
    def get_action(
        ...
        **kwargs,
                ...
                [
                    torch.zeros(
                        (
                            ...
                            self.max_length - action_masks.shape[1],
                            self.act_dim,
                        ),
        ...
        if visualize_logits is not None:
            ...
            rule_heads = [
                f'{rule.symbol.name}->{" ".join([symbol.name for symbol in rule.rhs])}'
                for rule in env.unwrapped.rules
            ]
            symbols = [symbol.name for symbol in env.unwrapped.symbols]
            last_action_pred_probs = (
                torch.nn.functional.softmax(last_action_pred_logits, dim=0)
                .detach()
                .cpu()
                .numpy()
                .reshape(env.unwrapped.num_rules, env.unwrapped.max_len)
            )

            fig, ax = plt.subplots(1, 1)
            im, cbar = heatmap(
                last_action_pred_probs[:, : len(env.unwrapped.symbols)],
                rule_heads,
                symbols,
                ax=ax,
                cmap="magma",
        ...
        if self.use_max_log_prob:
            ...
            last_action_pred_probs = torch.nn.functional.softmax(
                last_action_pred_logits, dim=0
            )
            action = torch.multinomial(
                last_action_pred_probs, num_samples=1, generator=self.generator
            ).squeeze()
